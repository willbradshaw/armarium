...
class UniqueLoader(yaml.SafeLoader):
    """Reject duplicate mappings instead of silently retaining the last value."""
...
def normalize(value: Any, ancestors: frozenset[int] = frozenset()) -> Any:
    ...
    if isinstance(value, (dict, list)):
        ...
        if isinstance(value, list):
            return [normalize(item, ancestors) for item in value]
        ...
        if any(not isinstance(key, str) for key in value):
            raise ValueError("frontmatter mapping keys must be strings")
        return {key: normalize(item, ancestors) for key, item in value.items()}
...
def parse(path: Path, root: Path) -> tuple[Note | None, list[Diagnostic]]:
    ...
    try:
        ...
        if not lines or lines[0].strip() != "---":
            return Note(path, {}, text, 1), []
        end = next((i for i in range(1, len(lines)) if lines[i].strip() == "---"), None)
        if end is None:
            raise ValueError("frontmatter has no closing --- delimiter")
        data = normalize(yaml.load("".join(lines[1:end]), Loader=UniqueLoader))
        if data is None:
            data = {}
        ...
        return None, [
            Diagnostic(
                relative, "parse.invalid", str(exc), line=mark.line + 2 if mark else 0
            )
        ]
