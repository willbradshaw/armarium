...
# -----------------------------------------------------------------------------
# Reading metadata and Markdown
# -----------------------------------------------------------------------------


def iter_text_values(value: object, field: str = "") -> Iterator[tuple[str, str]]:
    """Walk normalized metadata strings without coercing other scalar values.

    Args:
        value: JSON-compatible metadata to inspect.
        field: Optional field prefix for diagnostic locations.

    Yields:
        tuple[str, str]: Dotted field/index path and string value, in input order.
    """
    if isinstance(value, str):
        yield field, value
    elif isinstance(value, dict):
        for key, item in value.items():
            yield from iter_text_values(item, f"{field}.{key}" if field else str(key))
    elif isinstance(value, list):
        for index, item in enumerate(value):
            yield from iter_text_values(
                item, f"{field}.{index}" if field else str(index)
            )


def iter_markdown_lines(body: str) -> Iterator[tuple[int, str]]:
    """Read lines containing prose or literal links in executable queries.

    Args:
        body: Markdown body without frontmatter.

    Yields:
        tuple[int, str]: One-based body line and text with illustrative code
            excluded. Dataview/DataviewJS fences and inline =/$= expressions
            retain literal links; no queries are executed. Fence delimiters are
            omitted. Source line numbers are preserved.
    """
    fence = ""
    length = 0
    executable = False
    for number, line in enumerate(body.splitlines(), 1):
        ...
        if marker:
            ...
            if not fence:
                fence, length = delimiter[0], len(delimiter)
                executable = info.strip().lower() in {"dataview", "dataviewjs"}
            elif (
                delimiter[0] == fence and len(delimiter) >= length and not info.strip()
            ):
        ...
        if fence:
            if executable:
                yield number, line
            continue
        # Inline code is illustrative unless it is an executable Dataview expression.
        for span in reversed(list(re.finditer(r"(`+)(?!`)(.*?)\1(?!`)", line))):
            if not span[2].lstrip().startswith(("=", "$=")):
                line = line[: span.start()] + " " * len(span[0]) + line[span.end() :]
        yield number, line
