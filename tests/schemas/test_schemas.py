...
class SchemaTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.schemas = {name: json.loads((SCHEMAS / f'{name}.schema.json').read_text())
                       for name in MAPPING.values()}
        cls.validators = {name: Draft202012Validator(
            schema, format_checker=FormatChecker(), registry=Registry(retrieve=no_retrieval))
            for name, schema in cls.schemas.items()}
        cls.fixtures = {name: json.loads((Path(__file__).parent / 'fixtures' / f'{name}.json').read_text())
                        for name in MAPPING.values()}
    ...
    def test_mirrored_schemas_and_references(self):
        for folder in ('schemas', 'types', 'statuses'):
            ...
            self.assertEqual({p.name for p in starter.iterdir()}, {p.name for p in example.iterdir()})
