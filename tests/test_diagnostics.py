...
def test_failure_depends_on_error_severity(
    severities: list[Literal["error", "warning", "info"]], failed: bool
) -> None:
    result = Result(
        diagnostics=[
...
def test_results_accumulate_findings_independently() -> None:
...
def test_diagnostics_sort_deterministically_without_losing_locations() -> None:
